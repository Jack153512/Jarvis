...
class ProjectManager:
    ...
    def _safe_copy_file(self, source_path: str, dest_path: Path):
        ...
        try:
            ...
            raise OSError(f"safe_copy: write_tmp failed: {type(e).__name__}: {e}")
        ...
        try:
            ...
            try:
                ...
                raise OSError(f"safe_copy: replace failed: {type(e).__name__}: {e}; direct_overwrite failed: {type(e2).__name__}: {e2}")
            finally:
                try:
                    if tmp.exists():
                        tmp.unlink()
    ...
    def save_cad_artifact(self, source_path: str, prompt: str):
        ...
        ascii_prompt = normalized.encode("ascii", "ignore").decode("ascii")
        safe_prompt = "".join([c for c in ascii_prompt if c.isalnum() or c in (" ", "-", "_")])
        ...
        try:
            ...
            print(f"[ProjectManager] [WARN] Could not create cad dir: {cad_dir} ({type(e).__name__}: {e})")

        dest_path = cad_dir / filename
        
        try:
            self._safe_copy_file(source_path, dest_path)
            print(f"[ProjectManager] Saved CAD artifact to: {dest_path}")
            return str(dest_path)
        except Exception as e:
            print(f"[ProjectManager] [WARN] Failed to save artifact: {type(e).__name__}: {e}")
            fallback_dir = Path(tempfile.gettempdir()) / "jarvis_cad"
            try:
                fallback_dir.mkdir(parents=True, exist_ok=True)
                fallback_path = fallback_dir / filename
                self._safe_copy_file(source_path, fallback_path)
                print(f"[ProjectManager] [WARN] Saved CAD artifact to fallback path: {fallback_path}")
                return str(fallback_path)
            except Exception as e2:
                print(f"[ProjectManager] [ERR] Failed to save artifact to fallback: {type(e2).__name__}: {e2}")
                return None
